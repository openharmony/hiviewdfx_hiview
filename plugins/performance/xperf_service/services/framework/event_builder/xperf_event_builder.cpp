...
void XperfEventBuilder::SetParamName(HiSysEventParam& param, const char* name)
{
    ...
    if (ret != 0) {
        LOGE("XperfEventBuilder::SetParamName error, err is %{public}d.", ret);
    }
}
...
XperfEventBuilder& XperfEventBuilder::Param(const char* name, const std::string value)
{
    ...
    if (ret != 0) {
        LOGE("XperfEventBuilder::InsertParam string error, err is %{public}d.", ret);
    }
...
{
    if (evtName.empty()) {
        LOGE("[XperfEventBuilder::Check] evtName is empty");
        Release();
        return false;
    }
    if (paramList.size() == 0) {
        LOGE("[XperfEventBuilder::Check] paramList is empty");
        return false;
    }
    return true;
}
...
{
    LOGD("[XperfEventBuilder::Release]");
