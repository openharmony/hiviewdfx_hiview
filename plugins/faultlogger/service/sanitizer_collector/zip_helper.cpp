...
bool IsGZipFile(const std::string& sfilename)
{
    std::string rp = sfilename;
    if (IsLinkFile(sfilename)) {
        GetRealPath(sfilename, rp);
    }

    std::string needle(GZ_SUFFIX);
    std::string::iterator found = std::search(rp.rbegin(), rp.rend(),
                                              needle.rbegin(),
                                              needle.rend()).base();

    return found != rp.begin();
}
...
bool ReadGZFile(const std::string& gzfn, std::string& out)
{
    ...
    if (!gzfp) {
        SANITIZERD_LOGE("failed open gzfile error:%{public}s fn:%{public}s", strerror(errno), gzfn.c_str());
        return false;
    }
    ...
    while ((len = gzread(gzfp, buf, GZ_BUF_SIZE)) > 0) {
        out.append(std::string(buf), std::min(len, maxSize - size));
...
bool ReadFileToString(const std::string& path, std::string& out)
{
    if (IsGZipFile(path)) {
        ...
        return ReadGZFile(realfpath, out);
    }
...
static bool WriteNewFile(const int32_t fd, const T_SANITIZERD_PARAMS *params)
{
    ...
    std::time_t timeStamp = static_cast<time_t>(params->happenTime / MILLISEC_OF_PER_SEC);
