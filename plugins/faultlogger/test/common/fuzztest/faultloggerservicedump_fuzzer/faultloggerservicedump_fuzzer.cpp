...
#include "faultloggerservicedump_fuzzer.h"
#include "constants.h"
...
void FuzzServiceInterfaceDump(const uint8_t* data, size_t size)
{
    ...
    errno_t err = strncpy_s(reinterpret_cast<char*>(arg), sizeof(arg), reinterpret_cast<const char*>(data), maxLen - 1);
    if (err != EOK) {
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::FuzzServiceInterfaceDump(data, size);
    return 0;
}
