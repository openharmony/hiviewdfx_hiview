...
#include "faultlogcjerrorfillfaultloginfo_fuzzer.h"
...
void FuzzCjErrorFillFaultLogInfo(const uint8_t* data, size_t size)
{
    ...
    (void)cjError.FillFaultLogInfo(*sysEvent);
    cjError.info_.reason = sysEvent->GetEventValue(FaultKey::REASON);
    cjError.info_.logPath = "/data/test/fuzz_cjerror_log";
    cjError.UpdateFaultLogInfo();
    (void)cjError.GetFaultModule(*sysEvent);
    cjError.UpdateSysEvent(*sysEvent);
    (void)cjError.ReportToAppEvent(sysEvent);
}
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::FuzzCjErrorFillFaultLogInfo(data, size);
    return 0;
}
