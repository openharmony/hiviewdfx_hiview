 ...
 * @tc.desc: add app display mode change events to db.
 * @tc.type: FUNC
 */
HWTEST_F(FoldAppUsageTest, FoldAppUsageTest011_DisplayModeChange_EnterExitCoordination, TestSize.Level1)
{
    std::string workPath = "/data/test/";
    FileUtil::ForceRemoveDirectory(workPath + "sys_event_logger/", true);
    
    FoldEventCacher cacher(workPath);
    
    SysEventCreator focusCreator("WINDOWMANAGER", "FOCUS_WINDOW", SysEventCreator::BEHAVIOR);
    focusCreator.SetKeyValue("BUNDLE_NAME", "test.coordination.app");
    focusCreator.SetKeyValue("time_", 1000);
    auto focusEvent = std::make_shared<SysEvent>("test", nullptr, focusCreator);
    cacher.ProcessEvent(focusEvent);
    
    SysEventCreator enterCreator("WINDOWMANAGER", "DISPLAY_MODE", SysEventCreator::BEHAVIOR);
    enterCreator.SetKeyValue("FOLD_DISPLAY_MODE", FOLD_DISPLAY_MODE_COORDINATION); // 4
    enterCreator.SetKeyValue("time_", 2000);
    auto enterEvent = std::make_shared<SysEvent>("test", nullptr, enterCreator);
    cacher.ProcessEvent(enterEvent);
    
    FoldAppUsageDbHelper dbHelper(workPath);
    int enterIndex = dbHelper.QueryRawEventIndex("test.coordination.app", FoldEventId::EVENT_ENTER_COORDINATION_MODE);
    EXPECT_NE(enterIndex, 0);
    
    SysEventCreator exitCreator("WINDOWMANAGER", "DISPLAY_MODE", SysEventCreator::BEHAVIOR);
    exitCreator.SetKeyValue("FOLD_DISPLAY_MODE", 1);
    exitCreator.SetKeyValue("time_", 3000);
    auto exitEvent = std::make_shared<SysEvent>("test", nullptr, exitCreator);
    cacher.ProcessEvent(exitEvent);
    
    int exitIndex = dbHelper.QueryRawEventIndex("test.coordination.app", FoldEventId::EVENT_EXIT_COORDINATION_MODE);
    EXPECT_NE(exitIndex, 0);
    
    FileUtil::ForceRemoveDirectory(workPath + "sys_event_logger/", true);
    ASSERT_TRUE(!FileUtil::FileExists(workPath + "sys_event_logger/"));
}

/**
 * @tc.name: FoldAppUsageTest012
 * @tc.desc: test coordination duration.
 * @tc.type: FUNC
 */
HWTEST_F(FoldAppUsageTest, FoldAppUsageTest012_CoordinationDurationAccumulation, TestSize.Level1)
{
    std::string workPath = "/data/test/";
    FileUtil::ForceRemoveDirectory(workPath + "sys_event_logger/", true);
    ...
    SysEventCreator enter1Creator("WINDOWMANAGER", "DISPLAY_MODE", SysEventCreator::BEHAVIOR);
    enter1Creator.SetKeyValue("FOLD_DISPLAY_MODE", FOLD_DISPLAY_MODE_COORDINATION);
    enter1Creator.SetKeyValue("time_", 2000);
    auto enter1Event = std::make_shared<SysEvent>("test", nullptr, enter1Creator);
    cacher.ProcessEvent(enter1Event);
    
    SysEventCreator exit1Creator("WINDOWMANAGER", "DISPLAY_MODE", SysEventCreator::BEHAVIOR);
    exit1Creator.SetKeyValue("FOLD_DISPLAY_MODE", 1);
    exit1Creator.SetKeyValue("time_", 5000);
    auto exit1Event = std::make_shared<SysEvent>("test", nullptr, exit1Creator);
    cacher.ProcessEvent(exit1Event);
    
    SysEventCreator enter2Creator("WINDOWMANAGER", "DISPLAY_MODE", SysEventCreator::BEHAVIOR);
    enter2Creator.SetKeyValue("FOLD_DISPLAY_MODE", FOLD_DISPLAY_MODE_COORDINATION);
    enter2Creator.SetKeyValue("time_", 6000);
    auto enter2Event = std::make_shared<SysEvent>("test", nullptr, enter2Creator);
    cacher.ProcessEvent(enter2Event);
    
    SysEventCreator exit2Creator("WINDOWMANAGER", "DISPLAY_MODE", SysEventCreator::BEHAVIOR);
    exit2Creator.SetKeyValue("FOLD_DISPLAY_MODE", 1);
    exit2Creator.SetKeyValue("time_", 9000);
    auto exit2Event = std::make_shared<SysEvent>("test", nullptr, exit2Creator);
    cacher.ProcessEvent(exit2Event);
    
    FoldAppUsageDbHelper dbHelper(workPath);
    uint64_t startTime = 0;
    uint64_t endTime = 10000;
    std::unordered_map<std::string, FoldAppUsageInfo> infos;
    dbHelper.QueryStatisticEventsInPeriod(startTime, endTime, infos);
    
    uint32_t coordinationDuration = 0;
    for (const auto& pair : infos) {
        if (pair.second.package == "test.coordination.app") {
            coordinationDuration = pair.second.foldDisplayCoordination;
            break;
        }
    }
    EXPECT_EQ(coordinationDuration, 6000);
