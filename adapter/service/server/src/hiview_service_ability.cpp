...
ErrCode HiviewServiceAbility::DumpSnapshotTrace(int32_t caller, int32_t& errNo, std::vector<std::string>& files)
{
...
ErrCode HiviewServiceAbility::RecoverTrace(int32_t& errNo, int32_t& ret)
{
    if (!HasAccessPermission(DUMP_PERMISSION) && !HasAccessPermission(WRITE_HIVIEW_SYSTEM_PERMISSION)) {
        return HiviewNapiErrCode::ERR_PERMISSION_CHECK;
    }
