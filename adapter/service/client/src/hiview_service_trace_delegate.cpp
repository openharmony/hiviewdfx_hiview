...
{
    auto proxyHandler = [caller] (
        const sptr<IRemoteObject>& remote, CollectResult<std::vector<std::string>>& collectResult, int32_t& errNo) {
        return HiviewServiceAbilityProxy(remote).DumpSnapshotTrace(caller, errNo, collectResult.data);
    };
...
CollectResult<int32_t> HiViewServiceTraceDelegate::Recover()
{
    auto proxyHandler = [] (
        const sptr<IRemoteObject>& remote, CollectResult<int32_t>& collectResult, int32_t& errNo) {
        return HiviewServiceAbilityProxy(remote).RecoverTrace(errNo, collectResult.data);
    };
    return TraceCalling<int32_t>(proxyHandler);
}
