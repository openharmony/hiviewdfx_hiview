...
#ifndef HIVIEW_PLUGINS_SYS_EVENT_LOGGER_EVENT_DB_HELPER_H
#define HIVIEW_PLUGINS_SYS_EVENT_LOGGER_EVENT_DB_HELPER_H
...
public:
    ...
    int InsertSysUsageEvent(const std::shared_ptr<LoggerEvent>& event);
    int QueryPluginStatsEvent(std::vector<std::shared_ptr<LoggerEvent>>& events, const std::string& pluginName = "");
    int QuerySysUsageEvent(std::vector<std::shared_ptr<LoggerEvent>>& events);
    int DeletePluginStatsEvent();
    int DeleteSysUsageEvent();

private:
    void InitDbPath(const std::string& workPath);
    void InitDbStoreMgr();
...
#endif // HIVIEW_PLUGINS_SYS_EVENT_LOGGER_EVENT_DB_HELPER_H
